...
public class State implements Comparable<Object> {
    ...
    public void initializeChildren() {
    	//the purpose of this method is to get all possible child states.
    	Move[] moves = this.board.getPossibleMoves(player);
    	if(moves.length == 0) {
    		//No possible moves
    		return;
    	}
    	this.children = new State[moves.length];
    	for(int i = 0;i < moves.length;i++) {
    		Move move = moves[i];
    		Board b = new Board(this.board);
    		b.makeMove(move);
    		State child = new State(player, b, move);
    	}
    }
